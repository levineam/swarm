 ...
 */
export const createFeedGeneratorRecord = async (
  agent: AtpAgent,
): Promise<string> => {
  try {
 ...
 */
export const checkFeedGeneratorExists = async (
  agent: AtpAgent,
): Promise<boolean> => {
  try {
...
// Filter posts for Swarm community members
export async function filterPostsBySwarmCommunity(
  posts: FeedPost[],
): Promise<FeedPost[]> {
  return posts.filter(post => SWARM_COMMUNITY_MEMBERS.includes(post.author.did))
}
