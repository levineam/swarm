...
if (!newDid) {
  ...
  console.log('Example: node add-community-member.js did:plc:abcdefg123456 username.bsky.social');
  process.exit(1);
}
...
const communityMembersPath = path.join(__dirname, '../feed-generator/src/swarm-community-members.ts');
...
try {
  ...
  const arrayMatch = content.match(/export const SWARM_COMMUNITY_MEMBERS: string\[\] = \[([\s\S]*?)\]/);
  
  if (!arrayMatch) {
    console.error('Error: Could not find the SWARM_COMMUNITY_MEMBERS array in the file.');
    process.exit(1);
  }
  ...
  const updatedContent = content.replace(arrayMatch[0], `export const SWARM_COMMUNITY_MEMBERS: string[] = [${newArrayContent}\n]`);
  ...
  console.log('   git add ../feed-generator/src/swarm-community-members.ts');
