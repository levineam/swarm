...
const run = async () => {
  ...
  if (!handle || !password) {
    ...
    console.error('BLUESKY_HANDLE=your.handle BLUESKY_PASSWORD=your_password npx ts-node scripts/updateFeedGenDidNonInteractive.ts')
    process.exit(1)
  }
  ...
  try {
    ...
    console.log(`The feed generator DID has been updated from ${oldDid} to ${feedGenDid}`)
    console.log('Please allow a few minutes for the changes to propagate through the network.')
  } catch (error) {
